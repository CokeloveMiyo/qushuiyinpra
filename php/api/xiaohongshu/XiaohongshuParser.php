    ...
    private function formatNoteData($note)
    {
        ...
        if (!empty($note['imageList'])) {
            ...
            foreach ($note['imageList'] as $img) {
                ...
                // 仅当官方标记 livePhoto=true 时才当作实况动图（避免误把普通图当动图）
                $isLive = !empty($img['livePhoto']) || !empty($img['live_photo']);
                if (!$isLive) {
                    $stillCount++;
                    continue;
                }
            ...
            // 类型判定：
            // - 全部是实况 → live（动图）
            // - 存在静图 → image（照片），实况视频仍放在 live_photo 供可选下载
            // - 纯视频笔记保持 video
            if ($result['type'] !== 'video') {
                $liveCount = count($result['live_photo']);
                $imgCount = count($result['images']);
                if ($liveCount > 0 && $stillCount === 0 && $imgCount > 0) {
                    $result['type'] = 'live';
                } else {
                    $result['type'] = 'image';
                }
